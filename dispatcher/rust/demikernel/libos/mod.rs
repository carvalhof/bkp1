...
impl LibOS {
    ...
    /// Runs the Catnip LibOS
    #[cfg(feature = "catnip-libos")]
    pub fn run(&mut self, local: SocketAddrV4) {
        match self {
            LibOS::NetworkLibOS(libos) => libos.run(local),
    ...
    #[cfg(feature = "catnip-libos")]
    pub fn new(queue_id: u16, nr_queues: u16, mm: Arc<MemoryManager>) -> Result<Self, Fail> {
        logging::initialize();
        ...
        let config: Config = Config::new(config_path);
        let libos: LibOS = Self::NetworkLibOS(NetworkLibOS::Catnip(CatnipLibOS::new(&config, queue_id, nr_queues, mm)));
