...
impl TcpPeer {
    ...
    pub fn push2(&self, qd: QDesc, buf: DemiBuffer) -> Result<*mut rte_mbuf, Fail> {
        self.send2(qd, buf)
    }
    ...
    fn send2(&self, qd: QDesc, buf: DemiBuffer) -> Result<*mut rte_mbuf, Fail> {
        let inner = self.inner.borrow();
        let qtable = inner.qtable.borrow();
        match qtable.get(&qd) {
            Some(InetQueue::Tcp(ref queue)) => match queue.get_socket() {
                Socket::Established(ref socket) => socket.send2(buf),
