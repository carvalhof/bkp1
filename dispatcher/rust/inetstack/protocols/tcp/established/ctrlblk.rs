...
impl ControlBlock {
    ...
    pub fn send2(&self, buf: DemiBuffer) -> Result<*mut rte_mbuf, Fail> {
        self.sender.send2(buf, self)
    }
    ...
    pub fn emit2(&self, header: TcpHeader, body: Option<DemiBuffer>, remote_link_addr: MacAddress) -> *mut rte_mbuf {
        let segment = TcpSegment {
            ethernet2_hdr: Ethernet2Header::new(remote_link_addr, self.local_link_addr, EtherType2::Ipv4),
            ipv4_hdr: Ipv4Header::new(self.local.ip().clone(), self.remote.ip().clone(), IpProtocol::TCP),
            tcp_hdr: header,
        ...
        let body = segment.take_body().unwrap();

        let body_mbuf: *mut rte_mbuf = body.into_mbuf().expect("'body' should be DPDK-allocated");
