...
impl InetStack {
    ...
    pub fn do_push2(&mut self, qd: QDesc, buf: DemiBuffer) -> Result<*mut rte_mbuf, Fail> {
        match self.lookup_qtype(&qd) {
            Some(QType::TcpSocket) => self.ipv4.tcp.push2(qd, buf),
            Some(_) => Err(Fail::new(libc::EINVAL, "invalid queue type")),
            None => Err(Fail::new(libc::EBADF, "bad queue descriptor")),
        }
    }
    ...
    pub fn poll_bg_work(&mut self) -> Vec<u64> {
        #[cfg(feature = "profiler")]
