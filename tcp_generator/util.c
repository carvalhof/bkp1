	...
	for(uint64_t i = 0; i < nr_queues; i++) {
		...
		if(srv_distribution == UNIFORM_VALUE) {
			for(uint32_t j = 0; j < nr_elements_per_queue; j++) {
				application_array[i][j].iterations = srv_iterations;
				application_array[i][j].randomness = rte_rand();
			}
		} else {
			for(uint32_t j = 0; j < nr_elements_per_queue; j++) {
				double u = rte_drand();
				application_array[i][j].iterations = (uint64_t) (-((double)srv_iterations) * log(u));
				application_array[i][j].randomness = rte_rand();
			}
		}
	}
...
int app_parse_args(int argc, char **argv) {
	...
	while ((opt = getopt(argc, argvopt, "d:r:f:s:q:p:t:c:o:e:i:j:")) != EOF) {
		...
		case 'i':
			srv_iterations = process_int_arg(optarg);
			break;
