...
pub struct PushFuture<const N: usize> {
    pub cb: *mut ControlBlock<N>,
    pub err: Option<Fail>,
}

impl<const N: usize> fmt::Debug for PushFuture<N> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        unsafe { write!(f, "PushFuture({:?})", (*self.cb).qd) }
    }
}

impl<const N: usize> Future for PushFuture<N> {
    type Output = Result<(), Fail>;
...
pub struct PopFuture<const N: usize> {
    ...
    pub inner: Rc<RefCell<Inner<RECEIVE_BATCH_SIZE>>>,
}

impl<const N: usize> fmt::Debug for PopFuture<N> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        unsafe { write!(f, "PopFuture({:?})", (*self.cb).qd) }
    }
}
...
impl<const N: usize> Future for PopFuture<N> {
    ...
    fn poll(self: Pin<&mut Self>, ctx: &mut Context) -> Poll<Self::Output> {
        ...
        peer.poll_recv(self_.cb, ctx, size)
    }
}
