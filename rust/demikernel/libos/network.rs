...
impl NetworkLibOS {
    ...
    pub fn push(&mut self, cb: *mut ControlBlock<RECEIVE_BATCH_SIZE>, sga: &demi_sgarray_t) -> Result<QToken, Fail> {
        match self {
            ...
            #[cfg(feature = "catnip-libos")]
            NetworkLibOS::Catnip(libos) => libos.push(cb, sga),
            #[cfg(feature = "catloop-libos")]
    ...
    pub fn poll(&mut self) -> Vec<(*mut ControlBlock<RECEIVE_BATCH_SIZE>, QToken)> {
        match self {
            #[cfg(feature = "catpowder-libos")]
    ...
    pub fn get_rt(&self) -> Arc<DPDKRuntime> {
        match self {
            ...
            NetworkLibOS::Catcollar(libos) => libos.poll(),
            #[cfg(feature = "catnip-libos")]
            NetworkLibOS::Catnip(libos) => libos.get_rt(),
            #[cfg(feature = "catloop-libos")]
            NetworkLibOS::Catloop(libos) => libos.poll(),
        }
    }
    ...
    pub fn get_clock(&self) -> TimerRc {
        match self {
            ...
            NetworkLibOS::Catcollar(libos) => libos.poll(),
            #[cfg(feature = "catnip-libos")]
            NetworkLibOS::Catnip(libos) => libos.get_clock(),
            #[cfg(feature = "catloop-libos")]
            NetworkLibOS::Catloop(libos) => libos.poll(),
        }
    }
    ...
    pub fn get_scheduler(&self) -> Arc<Scheduler> {
        match self {
            ...
            NetworkLibOS::Catcollar(libos) => libos.poll(),
            #[cfg(feature = "catnip-libos")]
            NetworkLibOS::Catnip(libos) => libos.get_scheduler(),
            #[cfg(feature = "catloop-libos")]
            NetworkLibOS::Catloop(libos) => libos.poll(),
        }
    }
    ...
    pub fn get_ring(&self) -> *mut RingBuffer<(*mut rte_mbuf, *mut ControlBlock<RECEIVE_BATCH_SIZE>)> {
        match self {
            ...
            NetworkLibOS::Catcollar(libos) => libos.poll(),
            #[cfg(feature = "catnip-libos")]
            NetworkLibOS::Catnip(libos) => libos.get_ring(),
            #[cfg(feature = "catloop-libos")]
            NetworkLibOS::Catloop(libos) => libos.poll(),
        }
    }
