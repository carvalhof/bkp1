...
impl PassiveSocket {
    ...
    pub fn receive(&mut self, ip_header: &Ipv4Header, header: &TcpHeader) -> Result<(), Fail> {
        ...
        if self.inflight.contains_key(&remote) {
            if header.syn && !header.ack{
                ...
                match Self::reply(local_isn, remote_isn, self.local, remote, self.rt.clone(), self.tcp_config.clone(), self.local_link_addr, self.arp.clone()) {
                    Ok(_) => {},
                    Err(_) => return Err(Fail::new(EBADMSG, "Could not send the SYN+ACK")),
                }
    ...
    fn reply(
        ...
        let remote_link_addr = match arp.try_query(remote.ip().clone()) {
            Some(r) => r,
            None => return Err(Fail::new(EBADMSG, "Could not query the destination MAC")),
        };
